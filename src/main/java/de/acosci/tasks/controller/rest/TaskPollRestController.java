...
public class TaskPollRestController {
    ...
    @PreAuthorize("hasRole('ADMIN') or @taskSecurity.canEditTaskByEmail(#taskId, authentication.name)")
    public ResponseEntity<Void> sendInvitations(@PathVariable Long taskId) {
        taskPollMailService.sendInvitations(taskId);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/api/v1/tasks/{taskId}/poll/reminders/send")
    @PreAuthorize("hasRole('ADMIN') or @taskSecurity.canEditTaskByEmail(#taskId, authentication.name)")
    public ResponseEntity<Void> sendReminders(@PathVariable Long taskId) {
        taskPollMailService.sendReminders(taskId);
        return ResponseEntity.noContent().build();
    }
}
